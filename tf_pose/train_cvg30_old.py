...
if __name__ == '__main__':
    ...
    num_sample_valid = args.batchsize
    ...
    sample_valid = tf.placeholder(tf.float32, shape=(num_sample_valid, 640, 640, 3))
    train_img = tf.summary.image('training sample', sample_train, num_sample_train)
    valid_img = tf.summary.image('validation sample', sample_valid, num_sample_valid)
    ...
    with tf.Session(config=config) as sess:
        ...
        while True:
            ...
            if counter % 2000 == 0:
                # save weights
                saver.save(sess, os.path.join(modelpath, args.tag, 'model_latest'), global_step=counter)
